...
#' @param induce_sparsity  If \code{TRUE} the sparsity is induced on the network structure based on the values provided in the \code{density} argument.
...
#' @param irt If \code{TRUE}, data with \code{no_observations = 1000} from an IRT model is used to generate the maximum pseudolikelihood parameters which are then used to generate the simulated data sets for the MRFs (only relevant if \code{level = 'Discrete'}).
