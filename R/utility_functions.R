...
# Define a function to check positive definiteness
is_positive_definite <- function(mat) all(eigen(mat, only.values = TRUE)$values > 0)

# Define a function to validate sparsity
validate_sparsity <- function(K, graph) all(((zapsmall(K) != 0) == graph)[lower.tri(K)])
